...
/* The kernel hides the flexible-array members of struct pppoe_hdr and
 * struct pppoe_tag from in-kernel builds (they are #ifndef __KERNEL__ in
 * uapi/linux/if_pppox.h), so derive the addresses from the header instead. */
#define PPPOE_HDR_TAGS(ph)	((struct pppoe_tag *)((ph) + 1))
